...
def call_rva(patch_size=8, hidden_size=256, 
    num_glimpses=6, std=0.17, M=10, valid_size=0.1, batch_size=256, batchnorm_flag_phi=True,
    batchnorm_flag_l=True, batchnorm_flag_g=True, batchnorm_flag_h=True, weight_decay=0,
    ...
    config.loss_fun_action = 'cross_entropy';
    config.loss_fun_baseline = 'cross_entropy';
    ...
    config.data_dir = './data2/';
    config.ckpt_dir = './ckpt2/';
    config.logs_dir = './logs2/';
    ...
    # read best model
    if config.use_gpu:
        model_name = 'ram_gpu_{0}_{1}_{2}x{3}_{4}_{5:1.2f}_{6}_{7}_{8}_{9:1.2f}_{10:1.1f}_{11:1.1f}_{12:1.1f}_{13:1.1f}_{14}'.format(
                ...
                config.batchnorm_flag_g, config.weight_decay, 
                config.dropout_phi, config.dropout_l, 
                config.dropout_g, config.dropout_h, config.batch_size)                             
    else:
        model_name = 'ram_{0}_{1}_{2}x{3}_{4}_{5:1.2f}_{6}_{7}_{8}_{9:1.2f}_{10:1.1f}_{11:1.1f}_{12:1.1f}_{13:1.1f}_{14}'.format(
                config.PBSarray_ID, config.num_glimpses, 
                config.patch_size, config.patch_size,
                config.hidden_size, config.std, 
                config.batchnorm_flag_phi, config.batchnorm_flag_l,
...
def find_super_params():
    ...
    """
    # dicrete
    batch_size = inst.var.SoftmaxCategorical([128,256,512]) 
    ...
    hidden_size = inst.var.SoftmaxCategorical([128,256,512,1024]) 
    std = inst.var.SoftmaxCategorical(np.arange(0.1,0.6,0.1).tolist())
    ...
    instrum = inst.Instrumentation(patch_size, hidden_size, 
        num_glimpses, std, 10, 0.1, batch_size, 'True', 
        'True', 'True', 'True', 0,
        dropout_phi, 0, 0, dropout_h, 1.4, 0.9)   
    ...
    optimizer = optimizerlib.PortfolioDiscreteOnePlusOne(dimension=ifunc.dimension, 
        budget=20, num_workers=1) #TwoPointsDE
    # simple example
