...
class cluttered_MNIST(Dataset):
    ...
    def __getitem__(self, index): 
        ...
        # calcuate saliency map
        (_, saliencyMap) = self.saliency.computeSaliency((cluttered_img * 255).astype('uint8'))

        output_image = torch.zeros(2,height,width)
        output_image[0,...] = torch.from_numpy(cluttered_img)
        output_image[1,...] = torch.from_numpy(saliencyMap)
        transforms.functional.normalize(output_image, (0.1307,), (0.3081,))
        ...
        return (output_image, self.raw_dataset.__getitem__(index)[1])
