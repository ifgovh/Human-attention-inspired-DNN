...
class Trainer(object):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.use_gpu:
            self.model_name = 'ram_gpu_{0}_{1}_{2}x{3}_{4:1.2f}_{5}_{6}_{7}_{8}_{9}_{10:1.5f}_{11:1.1f}_{12:1.1f}_{13:1.1f}_{14:1.1f}'.format(
            ...
            self.model_name = 'ram_{0}_{1}_{2}x{3}_{4:1.2f}_{5}_{6}_{7}_{8}_{9}_{10:1.5f}_{11:1.1f}_{12:1.1f}_{13:1.1f}_{14:1.1f}'.format(
                config.PBSarray_ID, config.num_glimpses, 
                config.patch_size, config.patch_size,
                config.glimpse_scale, config.num_patches, 
                config.batchnorm_flag_phi, config.batchnorm_flag_l,
        ...
        self.model = RecurrentAttention(
            ...
            self.std, self.hidden_size, self.num_classes, config
        )
        if self.use_gpu:
            self.model.cuda()
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                if self.rnn_type == 'RNNCell':
                    ...
                    h_t, l_t, b_t, log_probas, p, cell_state = self.model(
                        x, l_t, h_t, last=True, cell_state_prev=cell_state
                    )
